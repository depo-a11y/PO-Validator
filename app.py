...
def validate_vendors(df):
    ...
    approved_vendors = [
        ...
        "DOUBLET", "DSM KEI NINOMIYA", "ENTIRE STUDIOS", "EGONLAB", 
        "FENG CHEN WANG", "FEAR OF GOD", "FEAR OF GOD ESSENTIALS", "JIL SANDER","JUUNJ",
        ...
        "VEJA", "WALES BONNER", "SONG FOR THE MUTE", "Y-3","WALD","HOUSE OF ERRORS","MM6 MAISON MARGIELA","UGG","HOKA"
    ]
...
def validate_data_and_log_errors(df):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            if cost > 0:
                if vendor_lower in ["veja", "taion","trudon","creed","dior","represent"]:
                    threshold = 2.19
                elif vendor_lower == "maison margiela":
                    threshold = 2.25
                else:
                    threshold = 2.5
